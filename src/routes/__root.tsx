    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          That page doesn't exist on Forecast Arena — or it's moved.
        </p>
        <div className="mt-6">
          <Link
            to="/"
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    ...
    links: [
      ...
      {
        rel: "stylesheet",
        href: "https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500;600&family=Fraunces:opsz,wght@9..144,400;9..144,500;9..144,600;9..144,700&display=swap",
      },
    ],
  }),
...
function RootComponent() {
  ...
  useEffect(() => {
    ...
    void import("@/integrations/supabase/client").then(({ supabase }) => {
      ...
      // Stash unsub on closure for cleanup
      (RootComponent as unknown as { _sub?: () => void })._sub = () =>
        sub.subscription.unsubscribe();
    });
    return () => {
      active = false;
      const cleanup = (RootComponent as unknown as { _sub?: () => void })._sub;
      cleanup?.();
    };
  }, [router, queryClient]);
