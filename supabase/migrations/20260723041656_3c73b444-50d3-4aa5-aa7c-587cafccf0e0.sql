...
-- Idempotent deposit credit invoked by the Paystack webhook (service_role only).
CREATE OR REPLACE FUNCTION public.record_deposit(
  p_user_id uuid,
  p_amount_kes numeric,
  p_provider_reference text,
  p_channel text DEFAULT 'mpesa'
) RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  v_entry_id uuid;
  v_idem text := 'paystack:deposit:' || p_provider_reference;
BEGIN
  IF p_amount_kes IS NULL OR p_amount_kes <= 0 THEN
    RAISE EXCEPTION 'invalid deposit amount';
  END IF;

  -- post_ledger_entry is idempotent on (user_id, idempotency_key)
  ...
  VALUES
    (p_user_id, 'collection', 'deposit', p_amount_kes, 'KES', 'paystack',
...
REVOKE ALL ON FUNCTION public.record_deposit(uuid, numeric, text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.record_deposit(uuid, numeric, text, text) TO service_role;

-- Finalize an M-Pesa transfer initiated by an admin (service_role only).
CREATE OR REPLACE FUNCTION public.finalize_payout(
  p_payout_id uuid,
  p_status text,             -- 'paid' or 'failed'
  p_provider_reference text,
...
BEGIN
  ...
  -- set_payout_status already handles ledger reversal on failure.
...
REVOKE ALL ON FUNCTION public.finalize_payout(uuid, text, text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.finalize_payout(uuid, text, text, text) TO service_role;
